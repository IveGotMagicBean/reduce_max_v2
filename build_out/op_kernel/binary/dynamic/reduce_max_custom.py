...
def _build_args(x_in__, y_out_, idx_out_, reduceDim, isKeepDim):
    __inputs__ = []
    for arg in [x_in__]:
        if arg != None:
    ...
    if reduceDim != None:
        ...
        attr["name"] = "reduceDim"
        attr["dtype"] = "int"
        attr["value"] = reduceDim
        __attrs__.append(attr)
    if isKeepDim != None:
        attr = {}
        attr["name"] = "isKeepDim"
        attr["dtype"] = "int"
        attr["value"] = isKeepDim
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("ReduceMaxCustom", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def reduce_max_custom(x_in__, y_out_, idx_out_, reduceDim, isKeepDim=1, kernel_name="reduce_max_custom", impl_mode = ""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_, idx_out_, reduceDim, isKeepDim)
    ...
    origin_func_name = "reduce_max_custom"
    ascendc_src_dir_ex = "reduce_max_custom"
    ascendc_src_dir = "reduce_max_custom"
    ascendc_src_file = "reduce_max_custom.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Ascend C Operator ReduceMaxCustom, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "ReduceMaxCustom"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__ , impl_mode = impl_mode, origin_inputs=[x_in__], origin_outputs = [y_out_, idx_out_],\
...
def op_select_format(x_in__, y_out_, idx_out_, reduceDim, isKeepDim=1, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_, idx_out_, reduceDim, isKeepDim)
    result = check_op_cap("op_select_format", "ReduceMaxCustom", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, y_out_, idx_out_, reduceDim, isKeepDim=1, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_, idx_out_, reduceDim, isKeepDim)
    result = check_op_cap("get_op_specific_info", "ReduceMaxCustom", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
