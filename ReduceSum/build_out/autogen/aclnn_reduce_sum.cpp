...
[[maybe_unused]] uint32_t NNOPBASE_ReduceSum = 0U;
} // namespace
    ...
    aclOpExecutor **executor)
{
    ...
    static NnopbaseDfxId tilingId = {0x60000, "aclnnReduceSumTiling", false};
    void *nnopExecutor;
    static void *executorSpace = NULL;
    const char *opType = "ReduceSum";
    char inputDesc[] = {1, 1};
    char outputDesc[] = {1};
    char attrDesc[] = {0, 0, 0};

    NNOPBASE_ASSERT_NOTNULL_RETVAL(x);
    NNOPBASE_ASSERT_NOTNULL_RETVAL(axes);
    ...
    *executor = reinterpret_cast<aclOpExecutor *>(nnopExecutor);
    ...
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddInput(*executor, axes, 1));
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddAttrWithDtype(*executor, static_cast<void*>(&keepDims), sizeof(bool), 0, kNnopbaseBool));
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddAttrWithDtype(*executor, static_cast<void*>(&ignoreNan), sizeof(bool), 1, kNnopbaseBool));
    if (dtypeOptional) {
        NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddAttrWithDtype(*executor, static_cast<void*>(dtypeOptional), strlen(dtypeOptional) + 1, 2, kNnopbaseString));
    } else {
        static char *dtypeOptionalDef = "float";
        NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddAttrWithDtype(*executor, static_cast<void*>(dtypeOptionalDef), strlen(dtypeOptionalDef) + 1, 2, kNnopbaseString));
    }
    ...
    if (NnopbaseAddParamName != NULL) {
        ...
        NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddParamName(*executor, 1, "axes", true));
